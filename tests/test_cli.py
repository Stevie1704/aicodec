...
def test_aggregate_command(mocker):
    ...
    mocker.patch.object(sys, 'argv', [
                        'aicodec', 'aggregate', '--include-ext', 'py', '--include-dir', '/tmp/project'])
    cli.main()
...
def test_apply_command(mocker):
    ...
    mocker.patch.object(
        sys, 'argv', ['aicodec', 'apply', '--output-dir', '.', '--changes', 'c.json'])
    cli.main()
...
def test_prepare_command_editor(tmp_path, mocker):
    ...
    mocker.patch.object(
        sys, 'argv', ['aicodec', 'prepare', '--changes', str(changes_file)])
    cli.main()
...
def test_prepare_command_from_clipboard(tmp_path, mocker):
    ...
    mocker.patch('builtins.open', mocker.mock_open(
        read_data=json.dumps(schema_content)))
    valid_json = json.dumps({"summary": "from clipboard", "changes": []})
    mock_pyperclip.return_value = valid_json
    changes_file = tmp_path / "changes.json"
    mocker.patch.object(sys, 'argv', [
                        'aicodec', 'prepare', '--from-clipboard', '--changes', str(changes_file)])
    cli.main()
